...
describe('Switch User from Non Member Dashboard', () => {
  beforeAll(async () => {
    const component = await DashboardPage();
    render(component);
  });
  it('should call update method of NextAuth correctly with router refresh on user switch', async () => {
    const userRaphel = screen.getByText('Raphel Claud');
    expect(userRaphel).toBeVisible();
    expect(screen.getByText('Robert Cook')).toBeVisible();
    expect(screen.getByText('Welcome, Alpha')).toBeVisible();
    // Switch role to self should not show
    ...
    fireEvent.click(userRaphel);
    await waitFor(() => {
      expect(unstable_update).toHaveBeenCalledWith({
        planId: null,
        userId: '57c85test3ebd23c7db88244',
      });
