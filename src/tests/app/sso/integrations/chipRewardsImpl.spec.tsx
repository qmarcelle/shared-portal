...
jest.mock('src/app/sso/ssoConstants', () => ({
  ...
  SSO_IMPL_MAP: new Map([['Chip_Rewards', 'ChipRewardsImpl']]),
}));
...
describe('Chip Rewards SSO', () => {
  beforeEach(() => {
    jest.resetAllMocks();
    mockedAxios.get.mockResolvedValueOnce({ data: loggedInUserInfoMockResp });
  });

  it('Should not route to Chip Rewards SSO when drop off service is failing', async () => {
    mockGet.mockReturnValueOnce('Chip_Rewards');
  ...
  it('Should route to Chip Rewards SSO when we click SSO Link', async () => {
    mockGet.mockReturnValueOnce('Chip_Rewards');
    mockGet.mockReturnValueOnce('');
