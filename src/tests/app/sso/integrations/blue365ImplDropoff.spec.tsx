...
process.env.NEXT_PUBLIC_DROP_OFF_IDP = 'Blue365';
...
describe('Blue365 SSO using Drop off API', () => {
  ...
  it('Should route to Blue 365 SSO when we click SSO Link', async () => {
    mockGet.mockReturnValueOnce('Blue365');
    mockGet.mockReturnValueOnce('');
    localAxios.post.mockResolvedValueOnce({
      data: { access_token: 'xyz-abc-test' },
    ...
    await waitFor(() => {
      ...
      expect(mockOpen).toHaveBeenCalledWith(
        `${process.env.NEXT_PUBLIC_PING_DROP_OFF_URL}spEntityId=Blue365&challenge=abcdef_l12345`,
        '_blank',
      );
    });
    expect(
