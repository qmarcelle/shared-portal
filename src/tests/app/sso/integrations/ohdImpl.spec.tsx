...
jest.mock('src/app/sso/ssoConstants', () => ({
  ...
  SSO_IMPL_MAP: new Map([['OHD', 'OHDImpl']]),
}));
...
describe('OHD SSO', () => {
  ...
  it('Should not route to OHD SSO when drop off service is failing', async () => {
    mockGet.mockReturnValueOnce('OHD');
  ...
  it('Should route to OHD SSO when we click SSO Link', async () => {
    mockGet.mockReturnValueOnce('OHD');
    mockGet.mockReturnValueOnce('');
