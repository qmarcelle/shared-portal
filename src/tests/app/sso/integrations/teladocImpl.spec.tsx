...
jest.mock('src/app/sso/ssoConstants', () => ({
  ...
  SSO_IMPL_MAP: new Map([['Teladoc', 'TeladocImpl']]),
}));
...
describe('Teladoc SSO', () => {
  ...
  it('Should route to Teladoc SSO when we click SSO Link', async () => {
    mockGet.mockReturnValueOnce('Teladoc');
  ...
  it('Should not route to Teladoc SSO when drop off service is failing', async () => {
    mockGet.mockReturnValueOnce('Teladoc');
    mockGet.mockReturnValueOnce('');
    localAxios.post.mockResolvedValueOnce({
      status: 400,
