import type { ChatEligibility, ChatInfoResponse } from '@/app/(protected)/(common)/member/chat/types/index';
    ...
    const fetchEligibility = async () => {
      try {
        const response = await fetch(
          `/a../((common)/chat/eligibility?memberId=${memberId}&planId=${planId}`,
        );
        if (!response.ok) {
          throw new ChatError(
