...
} from '@/app/(protected)/(common)/member/chat/types/index';
...
export class ChatService implements IChatService {
   ...
   *
   * API Endpoint: /a../((common)/chat/info
   * Response includes:
  ...
  async getChatInfo(): Promise<ChatInfoResponse> {
    try {
      const response = await memberService.get('/a../((common)/chat/info');
   ...
   *
   * API Endpoint: POST /a../((common)/chat/end
   *
   ...
   *
   * API Endpoint: POST /a../((common)/chat/message
   * Payload: { text: string }
   *
  ...
  async sendMessage(text: string): Promise<void> {
    try {
      const response = await fetch('/a../((common)/chat/message', {
        method: 'POST',
        headers: {
