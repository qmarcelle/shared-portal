...
interface SSOLaunchClientProps {
  partnerId: string;
  alternateSSOText?: string;
}

export function SSOLaunchClient({
  partnerId,
  alternateSSOText,
}: SSOLaunchClientProps) {
  ...
  const ssoImpl = SSO_IMPL_MAP.get(partnerId) ?? 'Not Found';
      ...
      if (!initialized.current) {
        ...
        try {
          ...
          if (isDropOffSSO(partnerId)) {
            const ref: string = await ssoDropOffToPing(ssoImpl);
  ...
  }, [partnerId, searchParams, ssoImpl]);
