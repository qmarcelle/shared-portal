...
export default function ChatProvider({
  ...
  useEffect(() => {
    ...
    if (
      ...
      // Raw plan objects from userProfileData.plans look like: { memCK, patientFhirId, selected }
      const plansFromProfile: MemberPlan[] = userProfileData.plans.map((p) => {
        const id =
          p.memCK ||
          p.patientFhirId ||
          `fallback-plan-id-${Math.random().toString(36).substring(7)}`;
        // For fields like name, subscriber, policies, lob - these are not directly on 'p'.
        // They either need to be sourced from elsewhere in userProfileData/pbeData or are truly static for this context.
        return {
          id: id,
          name: `Plan ${id}`, // Placeholder -  MUST BE ADJUSTED if a real name source exists
          subscriber: userProfileData.id || 'Unknown Subscriber', // Using userProfileData.id as a guess for subscriber context
          policies: [], // Placeholder - MUST BE ADJUSTED if policies are sourced elsewhere
          lob: undefined, // Optional in MemberPlan; explicitly undefined if not directly on p. Adjust if p can have .lob
        };
      });
      ...
      // Use the ID generated by the mapping above for consistency
      const selectedPlanInMappedArray = selectedPlanFromProfile
        ...
        : plansFromProfile[0]; // Default to first mapped plan if no specific selected one or if it couldn't be re-found
