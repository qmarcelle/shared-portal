...
// Add a new function to add emergency CSS styles that will display the button regardless of initialization
const addEmergencyCssOverrides = useCallback(() => {
  if (!document.getElementById('genesys-emergency-css-overrides')) {
    const styles = document.createElement('style');
    styles.id = 'genesys-emergency-css-overrides';
    styles.textContent = `
      /* Force all Genesys chat buttons to be visible regardless of initialization state */
      .cx-widget.cx-webchat-chat-button,
      .cx-webchat-chat-button,
      [data-cx-widget="WebChat"],
      .cx-button.cx-webchat,
      div[class*="cx-webchat"],
      button[class*="cx-webchat"],
      div[class*="cx-widget"][class*="chat-button"],
      div[class*="cx-button"] {
        display: block !important;
        visibility: visible !important;
        opacity: 1 !important;
      ...
      /* Make sure all children of buttons are visible too */
      .cx-widget.cx-webchat-chat-button *,
      .cx-webchat-chat-button *,
      [data-cx-widget="WebChat"] *,
      .cx-button.cx-webchat *,
      ...
      /* Override any transform containers that might affect fixed positioning */
      body * {
        transform-style: flat !important;
      }
      
      /* Make sure widget container doesn't overflow or clip the button */
      #genesys-chat-container,
      div[id*="cx-widget"],
      div[class*="cx-widget-container"] {
        overflow: visible !important;
        clip: auto !important;
        height: auto !important;
      }
      
      /* Add debugging outline so we can see the button even if content is missing */
      .cx-widget.cx-webchat-chat-button::after,
      .cx-webchat-chat-button::after,
      [data-cx-widget="WebChat"]::after,
      .cx-button.cx-webchat::after,
      div[class*="cx-webchat"]::after,
      button[class*="cx-webchat"]::after {
        content: "Chat";
        color: white;
        font-weight: bold;
    ...
    logger.info(
      `${LOG_PREFIX} Added emergency CSS overrides to force button visibility`,
    );
  }
}, []);

// Add a function to continuously check for and force-display the Genesys chat button
const activateEmergencyButtonFinder = useCallback(() => {
  logger.info(
    `${LOG_PREFIX} Setting up emergency button finder to ensure button visibility`,
  );

  // Immediate check
  const checkAndForceButtons = () => {
    // Use every possible selector that might match the chat button
    const selectors = [
      '.cx-widget.cx-webchat-chat-button',
      '.cx-webchat-chat-button',
      '[data-cx-widget="WebChat"]',
    ...
    if (foundButtons > 0) {
      logger.info(
        `${LOG_PREFIX} Emergency finder: Force-displayed ${foundButtons} chat button elements`,
      );
    }
  ...
  // Run immediately
  const initialCheck = checkAndForceButtons();
    ...
    mutations.forEach((mutation) => {
      ...
      mutation.addedNodes.forEach((node) => {
        if (node.nodeType === 1) {
          ...
          // Check if this node or any of its children might be a chat button
          if (
            el.classList?.contains?.('cx-widget') ||
            el.classList?.contains?.('cx-webchat-chat-button') ||
            el.querySelector?.('.cx-widget') ||
            el.querySelector?.('.cx-webchat-chat-button') ||
            el.getAttribute?.('data-cx-widget') === 'WebChat'
          ) {
            shouldCheck = true;
          }
        }
  ...
  // Also set up an interval to periodically check, just to be sure
...
// Call our emergency CSS function early in the component lifecycle
useEffect(() => {
  // Add emergency CSS as soon as possible
  addEmergencyCssOverrides();

  // Wait a moment to let the page load before starting the emergency finder
  const timerId = setTimeout(() => {
    const cleanup = activateEmergencyButtonFinder();
    return () => {
      cleanup();
...
}, [addEmergencyCssOverrides, activateEmergencyButtonFinder]);
