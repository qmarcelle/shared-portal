...
export const ViewPayPremium = ({
  ...
  const handleSSOConnection = async () => {
    ...
    try {
      // Simulate SSO connection request
      const response = await axios.post('/api/sso/connect', {
        partnerSpId: process.env.NEXT_PUBLIC_IDP_ELECTRONIC_PAYMENT_BOA,
      });

      if (response.data?.REF) {
        // Successful connection - navigate to SSO
        router.push(
