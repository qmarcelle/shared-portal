...
export default function ChatSession() {
  ...
  useEffect(() => {
    ...
    const onTyping = (event: any) => {
      ...
      const isTyping = 
        (typeof event?.typing === 'boolean') ? event.typing :
        (typeof event?.data?.typing === 'boolean') ? event.data.typing : 
        false;
    ...
    const messengerInterval = setInterval(() => {
      if (window.MessengerWidget && typeof window.MessengerWidget.on === 'function') {
        try {
          ...
          console.error('Error attaching to MessengerWidget typing indicator', err);
        }
      }
    }, 1000);
