...
import { Plan } from '@/lib/insurance/schemas';
...
export function BenefitsSection() {
  ...
  const { benefits, updateBenefits } = useBenefitsStore();
  ...
  return (
    <div>
      ...
      <div className="mb-8">
        ...
        <p className="mb-4">To keep your current benefits, select Keep Current Benefit from the table below.</p>
          ...
          <div className="overflow-x-auto mb-4">
            <table className="min-w-full border border-gray-300">
              <thead className="bg-blue-50">
                <tr>
                  <th className="px-4 py-2 border text-left">Keep Current Benefit</th>
                  <th className="px-4 py-2 border text-left">Plan Name (View Plan Details)</th>
                  ...
                  <th className="px-4 py-2 border text-left">Out of Pocket Max</th>
                  <th className="px-4 py-2 border text-left">Office Visit Copay</th>
              ...
              <tbody>
                <tr>
                  ...
                  <td className="px-4 py-2 border">{currentPlans.medical.metalLevel}</td>
                  <td className="px-4 py-2 border font-bold">{currentPlans.medical.network}</td>
                  <td className="px-4 py-2 border">
                    ${currentPlans.medical.deductible.toLocaleString()} Individual / ${(currentPlans.medical.deductible * 2).toLocaleString()} Family
                  </td>
                  <td className="px-4 py-2 border">
                    ${currentPlans.medical.outOfPocketMax.toLocaleString()} Individual / ${(currentPlans.medical.outOfPocketMax * 2).toLocaleString()} Family
                  </td>
                  <td className="px-4 py-2 border">{currentPlans.medical.officeVisitCopay}</td>
                  <td className="px-4 py-2 border">{currentPlans.medical.specialistCopay}</td>
                  <td className="px-4 py-2 border">{currentPlans.medical.rxCoverage}</td>
                </tr>
              </tbody>
            </table>
          </div>
            ...
            <tbody>
              {medicalPlans.map((plan) => (
                <tr key={plan.id}>
                  ...
                  <td className="px-4 py-2 border">
                    ${plan.deductible.toLocaleString()} Individual / ${(plan.deductible * 2).toLocaleString()} Family
                  </td>
                  <td className="px-4 py-2 border">
                    ${plan.outOfPocketMax.toLocaleString()} Individual / ${(plan.outOfPocketMax * 2).toLocaleString()} Family
                  </td>
