...
import { submitDetermination } from '../actions/submitDetermination';
...
const FormSubmittedStep = () => {
  ...
  const formData = useStore(DeterminationFormStore, (state) => state.formData);

  const submitForm = useCallback(async () => {
    try {
      setStatus('submitting');
      await submitDetermination(formData);
      setStatus('submitted');
    } catch (error) {
      setStatus('error');
    }
