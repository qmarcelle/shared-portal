...
export const LoginComponent = () => {
  ...
  const showTooltip = username.length < MIN_CODE_LENGTH && password.length < MIN_CODE_LENGTH;
  ...
  return (
    <form
      onSubmit={(e) => {
        username.length > 0 && password.length > 0 ? loginAnalytics(e) : undefined;
      }}
    >
      <div id="mainSection" className="dark:text-black">
        ...
        <Spacer size={32} />
        <div>
          <TextField label="Username" valueCallback={(val) => updateUsername(val)} />
          <Spacer size={32} />
          <TextField
            type="password"
            label="Password"
          ...
          >
            <Button
              callback={username.length > 0 && password.length > 0 ? loginAnalytics : undefined}
              style="submit"
