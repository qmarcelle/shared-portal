...
(function (window, document, $, cfg) {
  ...
  const setOptions = (optVar) => {
    ...
    switch (optVar) {
      case clientIdConst.BlueCare:
        ...
        if (cfg.isIDCardEligible === 'true' && cfg.isChatbotEligible === 'true')
  ...
  window._genesys.widgets.onReady = function (CXBus) {
    ...
    localWidgetPlugin.subscribe('WebChat.opened', function () {
      ...
      if (calculatedCiciId === clientIdConst.SeniorCare) {
        $('#question_field').hide();
        $('button[data-message="ChatFormSubmit"]').removeAttr('disabled').attr({
          id: 'startChat',
          class: 'cx-btn cx-btn-default i18n cx-btn-primary buttonWide',
        });
      }
  ...
  enableChatButton();
})(window, document, jQuery, window.chatSettings);
  ...
  const observer = new MutationObserver(function(mutations) {
    mutations.forEach(function(mutation) {
      if (mutation.type === 'childList') {
        ...
        const scriptNodes = addedNodes.filter(node => 
          node.nodeType === 1 && 
          node.tagName === 'SCRIPT' && 
...
function initializeChatButton() {
  ...
  if (!enableChatButton()) {
    // If enableChatButton fails, retry after a delay
    console.log('[Genesys] Retrying chat button initialization in 1 second...');
    setTimeout(initializeChatButton, 1000);
  }
}
...
function registerChatCommands() {
  if (window.CXBus && typeof window.CXBus.registerPlugin === 'function') {
    ...
    setTimeout(registerChatCommands, 1000);
  }
}
...
// Add a listener to ensure the button is enabled when the Genesys script loads
document.addEventListener('DOMContentLoaded', function() {
