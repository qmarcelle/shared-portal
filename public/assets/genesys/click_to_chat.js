(function (window, document) {
  ...
  const logger = {
    error: (...args) => console.error(...args), // Preserves console.error
    warn: (...args) => console.warn(...args), // Preserves console.warn
  };
...
<div class="modal" id="cobrowse-contactUsScreen2" tabindex="-1" role="dialog" style="display: none; z-index: 9999;">
  ...
  function initializeChatWidget($, currentWidgetCfg) {
    ...
    if (currentWidgetCfg.chatMode === 'legacy' && currentWidgetCfg.widgetUrl) {
      loadResource
        .script(currentWidgetCfg.widgetUrl, {
          id: 'genesys-widgets-min-script-dynamic',
        })
        .then(() => {
          // Script loaded, final checks for onReady presence
          if (
            !window._genesys ||
            ...
            logger.error(
              '[Genesys] FINAL CHECK FATAL: window._genesys.widgets.onReady is not a function or widgets object is missing after script load. Chat cannot initialize.',
            );
          } else {
            logger.warn(
              '[Genesys] Legacy widgets.min.js script load successful and onReady is configured.',
            );
          }
        })
        .catch((err) => {
    ...
    } else if (currentWidgetCfg.chatMode === 'cloud') {
      // Cloud mode initialization is typically handled by Genesys Messenger SDK snippet
    }
